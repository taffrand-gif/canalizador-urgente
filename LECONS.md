...
## Leçon #kanban-stale-cu-redispatch-2026-09-18-04 — 5ᵉ re-dispatch du même chantier SEO_PLAN : NO-OP légitime = résultat valide, pas échec

**Contexte** : entre le 2026-08-17 (`t_bb4ef8ea`, `t_33a93e6c`, `t_4a1bce6d`) et le 2026-09-18 (`t_318b6f69`), le même bloc `SEO_PLAN.md` lignes 119-127 (audit T₀ 2026-06-28 pré-A1 — homepage squelettique 16-39 éléments, grille prix absente, équipement non documenté, etc.) a été dispatché **5 fois** par le pool-keeper. Toutes les passes ont conclu à NO-OP légitime groupé après recompte live sur `origin/main`. `t_a07cc45d` (2026-09-01) a transformé 🔴→✅ via PR #312 squash `3d7200ddd` MERGED. `t_318b6f69` (2026-09-18) revalide J+17 sans modification SEO_PLAN. Aucun worker n'a produit de modification SEO_PLAN — toutes les traces sont allées dans JOURNAL.md.

**Takeaway** : un même chantier `SEO_PLAN.md` peut être re-dispatché N fois par le pool-keeper sans qu'aucun worker n'ait de modification SEO_PLAN à produire — **c'est attendu et correct**, pas un échec. Le coût d'une revalidation est nul (3 min recompte live), le coût d'une régression sur PR mergée est réel (contamination cascade de 3+ PRs). La trace JOURNAL.md suffit à prouver que la garde a été tenue.

**Action canon** :
1. **Pour les chantiers SEO_PLAN déjà marqués ✅ après passage CEO/docs-only**, accepter le NO-OP légitime comme **résultat valide**, pas comme échec. Pas de carte fille à matérialiser si la transformation 🔴→✅ a déjà été faite (vérifier `git log --grep="NO-OP.*l.119-127\|NO-OP.*ligne.*l.1[12][0-9]"` sur `origin/main` avant de proposer une nouvelle CEO/docs-only).
2. **Pas de drift sur PR mergée** : si `t_X` (date antérieure) a déjà produit la transformation ✅ et que sa PR est dans `origin/main`, ne pas re-transformer. La consigne SEO_PLAN est l'état final, pas un chantier ouvert.
3. **Métrique à reporter** : nombre de re-passages × durée recompte = coût de garde cumulable. Pour ce bloc : 5 passages × ~3 min = ~15 min cumulées. Coût marginal d'un faux positif de modification = contamination de 3 PRs en cascade. Le ratio plaide pour la **revalidation systématique** plutôt que le skip.
4. **Trace JOURNAL.md obligatoire** même pour les NO-OP : la trace prouve que la garde a été tenue, et le BOARD/CEO peut auditer a posteriori combien de fois le même chantier a été re-dispatché.
5. **Pattern à exposer au pool-keeper** (carte fille `cu-pool-redispatch-meta-2026-09-18`, assignee=`pool-keeper`, hors scope worker) : un chantier SEO_PLAN marqué ✅ après transformation CEO/docs-only peut être **automatiquement exclu** des re-dispatches suivants, SAUF si un commit sur `origin/main` touche les fichiers invariants (homepage `index.html`, JSON-LD `FAQPage`, etc.). Le pool-keeper peut implémenter un filtre `git diff origin/main -- index.html` qui re-dispatche SEULEMENT si le diff touche les invariants ; sinon, NO-OP silencieux.

**Source** : mission `t_318b6f69` (2026-09-18) sur canalizador-urgente/SEO_PLAN.md L.137 — recompte live `origin/main@06e77261a` (J+17 après `t_a07cc45d`) confirme les 5 invariants Doctrine §12 (prix canonique 65 €/h, Z1-Z6, +50%, orçamento por escrito, équipement Ridgid K9-102/ROLeak/FLIR E96) intacts + 8/8 pages /zonas/ prioritaires + Annexe A formulation collective (banni solo, formulation collective prime). Extension des leçons `kanban-stale-cu-homepage-count-2026-09-01-01/02/03` (créées par `t_bb4ef8ea`, `t_916db1b1`, `t_a07cc45d`). 1 fichier `JOURNAL.md` +19/-0 sur branche `docs/cu-seoplan-l137-t_318b6f69` depuis `origin/main`, PR draft #336 ouverte (https://github.com/taffrand-gif/canalizador-urgente/pull/336). **Zéro merge sans GO Filipe (R7).**
