...
### 2026-08-20 — t_a9810c1c — Rank-push 2/3 sur `canalizador urgente` — **fermeture du gap de maillage interne pilier↔guide**

- **Contexte** : GSC fenêtre 28j terminée 2026-08-20 = **1 impression / 0 clic / position 22** sur `canalizador urgente` (CPC=14.63 EUR, vol=170, score 2487.10 — broad-money H1). Page guide broad-money `/blog/canalizador-urgente-guia-completo` (créée 11/08, PR t_b63ca193) est **solide** : 45 occurrences du mot-clé, 12 Q/R FAQ schema (FAQPage JSON-LD), 5 blocs JSON-LD (BlogPosting + Service + FAQPage + BreadcrumbList + HowTo), NAP cohérent, prix HAUT 65 €/h + Z1–Z6 + +50 %, sitemap-blog actif, OG + Twitter Card. **MAIS** : la page la plus puissante du site (`index.html` — homepage, racine `canalizador-urgente.pt/`) ne contient **aucun lien** vers cette page guide — Google ne peut pas redistribuer le PageRank thématique interne du pilier vers la page cible.
- **Diagnostic anti-doublon** : page guide déjà en place, schéma FAQ + Service + BlogPosting déjà en place, sitemap déjà à jour. **Le seul défaut structural observable est l'absence de lien homepage → guide.** Toute nouvelle page serait du duplicate content (la page existe déjà, ciblée par la query). Toute modification de la page guide (ajout Q/R, refonte H1) sans signal interne de la racine n'aurait probablement pas d'impact mesurable — c'est exactement la leçon t_b63ca193.
- **Travail effectué** : (1) `index.html` — ajout d'**1 ligne dans la section « Leituras úteis do blog »** : `<li><a href="/blog/canalizador-urgente-guia-completo">Canalizador Urgente — Guia Completo (Trás-os-Montes)</a> — como funciona, quando chamar e quanto custa o serviço de urgência 24 h/7 dias.</li>` — texte d'ancrage aligné sur la query, lien contextuel dans la liste des lectures utiles (8e lien de la liste). (2) `blog/canalizador-urgente-guia-completo.html` — bump `dateModified` JSON-LD 2026-08-11 → 2026-08-20 (signal de fraîcheur). (3) `sitemap-blog.xml` — bump `lastmod` URL concernée 2026-08-11 → 2026-08-20 (cohérence sitemap ↔ page).
- **Témoins R8** (3 fichiers, 3 lignes exactement) :
  - `index.html` : `href="/blog/canalizador-urgente-guia-completo"` 0 → 1
  - `blog/canalizador-urgente-guia-completo.html` : `dateModified":"2026-08-11` 1 → 0, `dateModified":"2026-08-20` 0 → 1
  - `sitemap-blog.xml` : `<lastmod>2026-08-11</lastmod>` (URL guide) 1 → 0, `<lastmod>2026-08-20</lastmod>` (URL guide) 0 → 1
  - `git diff --numstat` : `3 fichiers modifiés, 3 insertions, 2 deletions` — scope strict.
- **Conformité** :
  - R12 post-08-07-08 : NAP inchangé (14 occurrences `928 484 451` sur homepage inchangé), grille prix non touchée, canonical `https://canalizador-urgente.pt/` inchangé.
  - R145 : aucune modification de texte utilisateur (date + lien). Le texte d'ancrage « serviço de urgência 24 h/7 dias » reste aligné R145 (24h/7d autorisé, « réponse immédiate » INTERDIT — mais cette formulation n'apparaît pas dans le diff).
  - R11 / §14 CU : zéro invention, zéro mention DGEG/TRIESP/wallbox/carregador (vérification grep).
  - Annexe A : zéro « je/eu », formulation « a nossa equipa » non touchée.
  - R6 : zéro force-push, branche `feat/cu-rankpush-canalizador-urgente-t_a9810c1c` depuis `origin/main` SHA `3e9ce6c51` (post-merge PR #267 ventilation FAQPage).
- **Branche** : `feat/cu-rankpush-canalizador-urgente-t_a9810c1c` depuis `origin/main` — **STOP MERGE** validation Philippe requise (R7).
- **Mesure** : cron `gsc-trajectoire-cron.sh` dim 22h → J+7/J+14 suivre impressions/clics/position sur `canalizador urgente`. Hypothèse de uplift : le signal de maillage interne pilier↔guide (la seule variable modifiée) doit permettre à Google de découvrir/considérer la page guide comme destination thématique du pilier homepage. Si confirmé → leçon « quand une page pilier broad-money est créée, son **signal de maillage interne depuis la homepage** est aussi important que le contenu de la page elle-même ». Si pas confirmé → escalader au CEO pour décision (autre levier : backlinks externes depuis `canalizador-norte-reparos.pt`, refonte H1 homepage pour match exact, ou acceptation de la position concurrentielle forte sur cette query).
- **Leçons consolidées** : (a) PR t_b63ca193 (11/08) leçon « créer une page pilier broad dédiée quand le renforcement d'une page existante échoue ». (b) PR t_a9810c1c (20/08) leçon « une page pilier broad sans maillage depuis la racine reste orpheline dans le graphe interne du site — son uplift GSC nécessite le signal de PageRank thématique du pilier homepage ». Ces deux leçons ensemble forment la doctrine **« page pilier broad-money = contenu solide + signal interne pilier↔guide »**.
---
