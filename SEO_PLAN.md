...
| T1-MONEY | Aligner H1 + sub homepage sur query money 'canalizador urgente' (CPC=14.63 EUR, vol=170, score=2487.10 — DFSEO 16/07) — **carte `t_ab64e870` 2026-09-25 : GAP live GSC 28j = 2 impr / 0 clic / pos 17 (snapshot recadré après push #335)** ; vague 2 = #337 (T2-MONEY-WEAK) | HAUTE | ⏸ DRAFT ⛔ **RÉGRESSION R12 PRIX** | [#335](https://github.com/taffrand-gif/canalizador-urgente/pull/335) + [#337](https://github.com/taffrand-gif/canalizador-urgente/pull/337) | GO Filipe (R7) | `canalizador[\s-]?urgente` (H1 l.258 + sub l.259) ; occurrences homepage 4 → 6 (+50 %) ; témoins R8 cf. JOURNAL.md (zone `t_be736a00`) ; PR #334 CLOSED (1ʳᵉ itération remplacée par #335 sur même branche, refacto push) ; vérification cible query 2026-09-25 (carte t_ab64e870) : `index.html` l.258 H1 = « Canalizador urgente 24 h/7 dias — resposta imediata em Trás-os-Montes » ✅ aligné ; `public/index.html` l.258 idem ✅ (sync copie conforme PR #67 squash `4144f002a`) ; NAP 928 484 451 homepage 15× ✅ ; JSON-LD `@context` schema.org ✅ (3 blocs `index.html` + 2 blocs `public/index.html` + 1 bloc `localizacoes/index.html` PARSE OK, `@context='https://schema.org'` — **dette SEO_PLAN l.128 FERMÉE** par `f31d2c73a`, pas de régression `436f1a4cc` ici) ; **⛔ RÉGRESSION PRIX R12 sur 3 homepages commit `436f1a4cc` 24/09 23:13** : `index.html` + `public/index.html` + `localizacoes/index.html` retombent en `70 €/h` + forfait unique 30 €/50 € (FAQPage + description JSON-LD + tableau localizacoes) au lieu de `65 €/h` + Z1-Z6 15 €-65 € Doctrine §12 ; 0 mention DGEG/TRIESP/wallbox/ficha eletrotécnica ✅ (vérifié grep -c = 0 sur 3 homepages) ; **PRs #335 + #337 NON mergeables en l'état actuel** : STOP_PENDING = (1) re-appliquer grille R12 prix canoniques sur 3 homepages + FAQPage JSON-LD (65 €/h ×5, Zona 1..6 ×21, +50% ×3, orçamento ×3) ; (2) re-vérifier témoins R8 Doctrine §12.1 prix HAUT avant demande GO Filipe ; R7 STOP MERGE obligatoire ; mesure J+7/J+14 via `gsc-trajectoire-cron.sh` dim 22h, baseline J+0 = 2 impr / 0 clic / pos 17, cible top 10 SERP (pos < 10) |
...
**Dernière MAJ** : 2026-09-25 — **carte `t_ab64e870` (T1-MONEY GAP live) — vérification cible query 2026-09-25 ✅** : `index.html` l.258 H1 = « Canalizador urgente 24 h/7 dias — resposta imediata em Trás-os-Montes » aligné query exacte ; `public/index.html` l.258 idem (sync copie conforme PR #67 squash `4144f002a`) ; NAP 928 484 451 cohérent 15× homepage ; JSON-LD `@context` schema.org ✅ (3 blocs `index.html` + 2 blocs `public/index.html` + 1 bloc `localizacoes/index.html` PARSE OK — dette SEO_PLAN l.128 FERMÉE par `f31d2c73a`, **pas de régression `436f1a4cc` ici**) ; **⛔ RÉGRESSION PRIX R12 détectée commit `436f1a4cc` 24/09 23:13** : 3 homepages (`index.html` + `public/index.html` + `localizacoes/index.html`) retombent en `70 €/h` + forfait unique 30 €/50 € (FAQPage + description JSON-LD + tableau localizacoes) au lieu de `65 €/h` + Z1-Z6 15 €-65 € Doctrine §12 ; 0 mention DGEG/TRIESP/wallbox/ficha eletrotécnica ✅ (vérifié grep -c = 0 sur 3 homepages) ; **PRs #335 (T1) + #337 (T2 vague 2) NON mergeables en l'état actuel** : STOP_PENDING = (1) re-appliquer grille R12 prix canoniques sur 3 homepages + FAQPage JSON-LD (65 €/h ×5, Zona 1..6 ×21, +50% ×3, orçamento ×3) ; (2) re-vérifier témoins R8 Doctrine §12.1 prix HAUT avant demande GO Filipe. R7 STOP MERGE — JAMAIS merger main sans STOP validation Filipe AGENTS.md §12 urgence. Snapshot J+0 = GSC 28j = 2 impr / 0 clic / pos 17 (recadré depuis 3 impr / 0 clic / pos 14.3 baseline 21/09) ; mesure J+7/J+14 via `gsc-trajectoire-cron.sh` dim 22h, cible top 10 SERP (pos < 10) — voir entrée CHANTIERS:BEGIN T1-MONEY enrichie ligne 35 ci-dessus pour détail témoins R8 (Doctrine §12.1 prix HAUT, NAP, équipement réel, FAQPage). Précédent : 2026-09-21 00:46 — PR #337 OPEN (5 commits, pushé) T2-MONEY-WEAK + JSON-LD schema.org RESTORE (commit `f31d2c73a`).
