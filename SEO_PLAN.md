...
**Dernière MAJ** : 2026-06-30 — **PR #60 (schema LocalBusiness homepage) ouverte + PR #61 (cleanup contactos.html JSON-LD + email) mergée squash `1d522782e`** + **BUG CRITIQUE DÉCOUVERT** : 1504 fichiers .html contiennent `70€/h` (tarif élec) au lieu de `65€/h` (tarif canal R12). Doctrine §12 cassée en prod depuis plusieurs jours, copier-coller hérité d'un PR antérieur non détecté par les audits. Audit GSC API : canalizador-urgente.pt = **4 pages indexées / 10 impressions / 0 clic en 28j** sur 1828 fichiers (~0,22%). 3 sub-agents en parallèle mode loop ont produit : sub-agent CU cleanup → PR #61 mergée · sub-agent EU cleanup → interrompu (T1 no-op, T2 alij.html OK, T3 contactos.html à finir) · sub-agent DOC → commits `8f4154fec` MAJ SEO_PLAN + INDEX_MULTI_SITES.md créé.
