...
| 2026-07-03 | Hermes | **D3 OSRM batch : 272 localités uniques → zonas-data.json étendu 914→958 (+44)** | Script `_audit/d3/d3_osrm_batch.py` (Nominatim + OSRM public, Macedo centre 41.5377,-6.9624). Pipeline : 619 fichiers NO_RESOL (= pages service×localité) → 272 localités uniques réelles → 64 géocodées Nominatim (24% succès — compositions service×localité) → 43 in_zone (Z1-Z6) + 21 OUT_OF_AREA (>130km) + 1 fallback_concelho + 175 fallback_skipped + 32 already_in_zonas. `zonas-data.json` étendu 914 → 958 entrées. | R3 (audit), R145 zéro délai (résolu par géocodage), R7 (OOA liste = décision CEO) | 21 OOA listées pour CEO (xinzo-de-limia 131km, distrito-de-guarda 136km, Espagne hors-zone : monterrei, albarellos, vern, vias, falde, argan, olas, mahde, ombra, a-gudia, etc.). D3-bis identifié : étendre `freguesia_concelho.json` (192 entrées) avec variantes service-prefix pour augmenter taux fallback. Snapshot : `_audit/d3/zonas-data_after.json`. | ⏳ 21 OOA en attente décision CEO |
