...
| T2-MONEY-WEAK | Renforcer homepage (WEAK sur 3 impr / 0 clic / pos 14.3 — même query broad money 'canalizador urgente') : +FAQPage JSON-LD 5 questions (close dette SEO_PLAN l.128), +section 'Situações que pedem um canalizador urgente' 385 mots visibles 6 cas typiques equipment réel, +6 liens internes /zonas/ (Bragança/Mirandela/Chaves/Macedo/Vila Real/Lamego — homepage avait 0 lien sortant) ; **+fix R145 'menos de duas horas'** +meta OG/Twitter/keywords +title 112→93 car avec 65 €/h (Doctrine §12.1 transparence HAUT) | HAUTE | ⏸ DRAFT | [#337](https://github.com/taffrand-gif/canalizador-urgente/pull/337) | GO Filipe (R7) | branche `fix/cu-t2-money-weak-canalizador-urgente-t_3a35e691` ; commits `5a7dce57c` (FAQPage+Situações+/zonas/) + `8b96006cb` (R145 fix + OG/Twitter + title — t_3a35e691) + `0be88052b` (docs SEO_PLAN) + **`f31d2c73a`** (JSON-LD schema.org restore — review t_348c33e6 P0 : les 2 blocs homepage avaient `https://***@type` corrompu au lieu de `https://schema.org`, JSON invalide, dette SEO_PLAN l.128 NON fermée ; fix 2/2 blocs parses OK, schema.org×2, `***@type` 2→0) + `a65f5cbc9` (docs SEO_PLAN consigne JSON-LD restore) ; homepage **494 lignes** / 2340 mots ; FAQPage JSON-LD 0→1 (5 questions, parse OK) + LocalBusiness restauré ; /zonas/ liens 0→6 ; H1+sub INCHANGES (PR #335) ; R145 'X horas' occurrences 1→0 ; **recompte live 2026-09-21 carte t_204fd406** : JSON-LD 2/2 blocs `https://schema.org` parses OK (Plumber/LocalBusiness/ProfessionalService + FAQPage) ; Doctrine §12.1 = 65 €/h×7, Z1=×2, +50%×3, orçamento por escrito×8 ; Equipment §12.6 = Ridgid K9-102×2, ROLeak Aqua 3Plus×2, câmara inspeção×1 ; NAP `+351 928 484 451`×2 + `+351****4451`×1 (tel: lien masqué R-TEL, hors-scope) ; R145 `menos de X horas`=0, `X minutos`=0 ; maillage 6/6 /zonas/ OK ; OG×9 + Twitter×5 + keywords×1 ajoutés ; `***@type` 0 (P0 fix complet) ; **PR #337 OPEN MERGEABLE Vercel SUCCESS** (5 commits, pushé, scope strict 3 fichiers : SEO_PLAN.md + _indexing/INDEXING-LOG.md + public/index.html ; attente GO Filipe pour merge — R7 STOP MERGE) |
<!-- CHANTIERS:END -->
