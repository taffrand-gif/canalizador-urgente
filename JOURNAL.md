...
## 🔄 HISTORIQUE
| 2026-08-29 | cowork-loop | **Rayon de couverture — le `<h2>` « Zonas Atendidas (Raio 100km) » d'un article premium contredisait les 130 km du reste du dépôt** | Le repo dit `raio … 130` sur **43 fichiers** et `raio … 100` sur **un seul** : `blog/canalizador-urgente-braganca-24h-premium.html`, dans un **`<h2>`** — donc un signal de structure lu par les moteurs, pas une mention perdue dans la prose. Le jumeau EU porte **le même `<h2>` au même défaut** (`blog/eletricista-urgente-braganca-24h-premium.html`) : **le gabarit de l'article « 24h premium » est commun aux deux sites d'urgence et n'a été repris par aucun des batchs de rayon**, qui ont tous porté sur les sites d'installation. ⚠️ **Non touché délibérément** : les 10 fichiers portant `Z4 70–100 km` / `Z5 (75-100 km)` — ce sont les **bornes de la grille de zones**, pas le rayon d'entreprise. 🔴 **Signalé pour un prochain run** : ces bornes ne sont pas homogènes entre les pages (`Z4 70–100` dans les tableaux de `precos.html` contre `Z4 (91-130 km)` cité par `contactos.html` au run du 27/08) — **famille distincte, à mesurer avant tout patch.** | R4 (zéro invention — valeur `130 km` verrouillée par `AGENTS.md` « rayon ~130 km autour de Macedo de Cavaleiros » et `PRICING.md` « Z6 = ~130 km route max », et déjà majoritaire en production), R11/R12 (contradiction active en production = priorité sur la tâche prévue), R8 (témoin avant/après, 1 motif par commande, compté en Python sur le contenu des fichiers), commit atomique 1 fichier = 1 commit, R6 (zéro force-push), R7 (zéro merge), R-WT (aucune copie de travail partagée touchée — commits construits par plumbing `commit-tree` sur un index temporaire, ni branche courante ni index ni fichier du checkout partagé modifiés) | **2 commits, 1 fichier de production** + `SEO_PLAN.md`. **Témoin R8** — périmètre : arbre complet de `origin/main` hors `_archive/ _audit/ _backlog/ docs/ node_modules/`, hors `.md` = **2633 fichiers** ; motif `[Rr]aio[^.<]{0,18}100` : **1 → 0** ; motif `…130` : **43 → 44 fichiers** ; scan détagué de rattrapage : **0**. `git diff --numstat` = **1/1**. **Diff octet à octet : exactement 1 octet, `0` → `3`.** Mots : `100km` → `130km`. Delta de balises **vide**. Branche `loop/2026-08-29-cu-raio-130km` depuis `origin/main` (`ae06435e8`). | ⏳ branche poussée, PR à ouvrir |
| 2026-08-27 | cowork-loop | **Rang 4 — `contactos.html` servait `Z1 (0-30 km): . Z2 (31-50 km): . Z3 (51-90 km): . Z4 (91-130 km): .` en clair sur une money page ; + rang 12 — les 2 variantes hybrides de `Quanto custa a deslocação?` ; + rang 11 livré ; + rang 1 CLOS** | 🟢 **Contrôle d'ouverture : `gh pr list --state open` rend 0 PR ouverte sur les 4 repos.** Sur CU, #264 → #276 ont toutes mergé. 🔴 **RANG 1 CLOS SANS RÉGRESSION** — le risque signalé **trois fois** (20/08, 24/08, 25/08) était : « si #268 merge après #269, la corruption `***` revient en production ». Vérification faite : **#268 a été FERMÉE sans merge** le 26/08, #269 a mergé le 26/08. Contrôle sur `blog/canalizador-urgente-guia-completo.html` : **0 occurrence de `***`**, **5 blocs `"@context":"https://schema.org"`, tous valides**. Le risque est éteint. **Rang 4** (débloqué par le merge de #264) : des valeurs de délai purgées au titre de R145 avaient laissé une énumération **amputée**, servie en clair au visiteur dans la FAQ « Quanto tempo até chegarem a minha zona? ». Aucun donneur dans le dépôt (**1 seule occurrence de cette réponse sur 2 534 fichiers**) ⇒ **reconstruire un délai serait une invention ET une violation de R145**. Correctif = **suppression pure** de l'énumération vide, la phrase porteuse `Depende da zona.` est conservée. **Zéro mot ajouté.** **Rang 12** : `"Z3: 35 € e 65 €/h de mão de obra."` répondait à « Quanto custa a **deslocação**? » en mélangeant le prix de déslocation et le taux horaire de main-d'œuvre — **deux grandeurs différentes**. Forme conforme transplantée **verbatim** du gabarit majoritaire : `"A zona é Z3: 35 € de deslocação, independentes da mão de obra."` — **45 donneurs** dans le dépôt, **une seule forme distincte**, dont **4 exactement en Z3**. Les 3 fichiers sont bien en Z3 à 35 € (`zona Z3` et `Z3 = 35 €` déjà présents 8 fois par fichier) : **zéro invention, la valeur est déjà dans le fichier**. | R4 (zéro invention — suppression pure sur `contactos.html` ; forme transplantée verbatim de 45 donneurs à forme unique sur les 3 autres), R145 (ne pas réintroduire de délai chiffré : c'est *pourquoi* le rang 4 se règle par retrait), R11/R12 (défauts actifs en production = priorité), R8 (témoin avant/après par fichier, 1 motif, compté **en Python sur le contenu des fichiers**), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree `~/work/Sites/_loop-2026-08-27/wt-cu` ; checkout partagé **non touché**) | **5 commits, 4 fichiers de production** + `SEO_PLAN.md`. **Témoin R8** — motif `Z\d \([^)]*\):\s*\.` : **4 → 0** sur `contactos.html`, **0 ailleurs** dans le dépôt. Motif `"Z3: 35 € e 65 €/h de mão de obra."` : **3 → 0** (`canalizador-gimonde`, `canalizador-gondesende`, `canalizador-meixedo`). **Contrôle « mot inexistant »** : sur `contactos.html` **aucun mot nouveau** (seuls disparaissent les libellés de l'énumération vide) ; sur les 3 autres, **un seul mot nouveau, `independentes`**, présent 45 fois chez les donneurs. **Contrôle de balises** : delta **vide** sur `contactos.html`. **Contrôle JSON-LD** (validité *et* typage) : **2 blocs valides par fichier, `@context` = `https://schema.org` intact après écriture** — patch en Python pur, cf. `LECONS.md` L#003. 🔴 **RANG 11 LIVRÉ — et il INFIRME l'extrapolation depuis CNR.** Périmètre énoncé : tout le dépôt **hors `_archive*`**, hors `_audit/ _backlog/ docs/`, hors les 6 `.md` de doctrine racine, **`blog/*.md` INCLUS** = **2 637 fichiers** ; fenêtre de **60 caractères**. **`orçamento`↔`gratuit` : 110 occurrences / 100 fichiers** — contre **4 701 / 2 037 sur CNR** et **3 700 / 1 678 sur ENR**. **Facteur ~40. L'hypothèse « le compteur de CU a la même origine » est FAUSSE.** 🔴 **Et deux familles bloquées par GO sont en réalité VIDES en production** : `Você` **23 occ. / 17 fic., 100 % dans des `.md`** ; corruption `*Parranj*` **31 occ. / 17 fic., 100 % dans des `.md`**. **Zéro occurrence dans le HTML servi.** ⚠️ Mais **27 des 103 `.md` ont un jumeau `.html`** : ce sont des **sources de génération**, donc les deux familles sont **latentes**, pas closes. **À router vers l'hygiène de source, pas vers la purge de production.** Autres mesures au même périmètre : `Diagnóstico por telefone em poucos minutos` **5 139 / 1 087** · `Atendimento 24h` **4 200 / 908** · `garantimos atenção` **249 / 190** (dont **14 seulement** en forme longue — le rang 5 sous-comptait d'un facteur 18). Branche `loop/2026-08-27-cu-contactos-zonas-orphelines` depuis `origin/main` (`1d7e08b94`). | ⏳ PR ouverte |
| 2026-08-25 | cowork-loop | **Rang 3 de la file — 11 meta descriptions à phrase dupliquée, purgées ; et l'hypothèse des « pages en double par accent » VÉRIFIÉE : 179 paires, 186 pages `noindex` dans les sitemaps, 7 concelhos sans aucune page indexable** | **Patch.** Prédicat exact : *segment de plus de 25 caractères présent deux fois dans la même `<meta name="description">`*. Balayage sur **2453 pages** hors `_archive*` : **11 pages** (et non 10 — `canalizador-urgente-carrazeda-de-ansiães.html` n'était pas dans la liste du 24/08). ⚠️ **Le premier prédicat écrit a rendu 0 : il découpait en PHRASES et cherchait deux phrases égales.** Or le segment répété est un **sous-segment**, la première occurrence portant un préfixe (« …45€ deslocação, » ) ou vivant **entre parenthèses** (carrazeda). Conformément à la doctrine « un contrôle qui rend 0 doit prouver sa source », le 0 a été vérifié contre le contenu réel d'une des pages citées, ce qui a exposé le défaut du prédicat. **Prédicat correct : plus long sous-segment répété, pas phrase répétée.** 3 formes distinctes : `Atendimento 24h/7 dias — ligue 928 484 451.` ×5 (+1 entre parenthèses) · `Diagnóstico por telefone em poucos minutos — ligue 928 484 451, garantimos atenção após contacto telefónico.` ×3 · `Diagnóstico por telefone em poucos minutos, orçamento por escrito antes da deslocação.` ×2. **Correction = suppression pure de la seconde occurrence**, aucun mot ajouté ni reformulé. Les 11 fichiers étaient **LIBRES** (contrôle `gh pr view <n> --json files` sur les 5 PR ouvertes, passé avant le calcul du périmètre). 🔴 **VERDICT — hypothèse des pages en double par accent : CONFIRMÉE et beaucoup plus grosse que prévu.** **179 paires** de pages dont les chemins sont identiques à l'accent près (`castelaos`/`castelãos`, `acoreira`/`açoreira`, `alijo`/`alijó`…), **aucune byte-identique** (md5 tous différents → la méthode de retrait mécanique EU #314 ne s'applique pas). Sur les 179 : **172 ont un seul jumeau en `noindex,follow`** (déduplication en place, correcte) et **7 ont LES DEUX jumeaux en `noindex`**. 🔴 **Ces 7 localités n'ont AUCUNE page indexable** : **Alijó · Carrazeda de Ansiães · Freixo de Espada à Cinta · Freixo de Numão · Mouçós · São João da Pesqueira · Tabuaço** — ce sont des **concelhos**, pas des hameaux. 🔴 **Et les 186 pages `noindex` issues de ces paires sont présentes dans les sitemaps — 186 sur 186**, dans les quatre : `sitemap.xml`, `sitemap-villages.xml`, `public/sitemap.xml`, `public/sitemap-villages.xml` (`sitemap-villages.xml` : 1998 `<loc>`, dont **180 accentuées**). C'est le signal « **Submitted URL marked 'noindex'** » de la Search Console : le sitemap soumet à l'indexation des pages qui l'interdisent. **Ni patché ni décidé ici — 186 suppressions dans 4 sitemaps générés = batch → GO périmètre.** | R4 (zéro invention — suppression pure d'un segment déjà dupliqué, aucun mot ajouté) ; R8 (témoin avant/après compté **en Python sur le contenu**, sur les 2453 pages) ; « un contrôle qui rend 0 doit prouver sa source » (appliqué, et il a effectivement révélé un prédicat faux) ; commit atomique 1 fichier = 1 commit ; R6 ; R7 (zéro merge) ; R-WT (worktree `~/work/Sites/_worktrees/loop-cu-20260825` ; checkout partagé **non touché**) | **11 commits, 11 fichiers de production.** Diff : **11 insertions, 11 suppressions**. **Témoin R8** — pages dont la `<meta name="description">` contient un segment >25 caractères répété : **11 → 0 sur 2453 pages balayées**. Branche `loop/2026-08-25-cu-meta-dupliquee` depuis `origin/main`. | ⏳ PR ouverte |
| 2026-08-24 | cowork-loop | **Rang 3 (sweep `LECONS.md` en étape fixe) — 27 propriétés `"type"` au lieu de `"@type"` dans les blocs JSON-LD de 11 pages, + 5 noms de Question corrompus par une purge** | Rang 1 toujours **pris par la PR #264** ; rang 2 (`vimioso`) reste **indéterminé** ; rangs 4 et 5 en attente de GO périmètre. Rang 3 exécuté. 🔴 **Le sweep a sorti un défaut qu'aucun compteur ne voyait, et que le balayage structurel du 23/08 ne pouvait pas voir non plus** : les blocs concernés sont **du JSON parfaitement valide** — ils passent `json.loads` — mais ils écrivent `"type":"Answer"` et `"type":"Organization"` au lieu de `"@type"`. **`schema.org` n'y reconnaît aucun type** : les nœuds `Answer` et `Organization` ne sont **pas rattachés au graphe**, donc les réponses de FAQ et les éditeurs de ces pages ne sont **pas émis**. **Validité JSON ≠ validité schema.org — ce sont deux contrôles différents, et le loop n'avait que le premier.** **Témoin d'unanimité** : après patch, `"type"` nu = **0 sur tout le dépôt** ; les 11 fichiers portent entre 22 et 29 `"@type"` chacun. Avant patch, **toutes** les autres occurrences du dépôt écrivaient déjà `"@type"` — la forme correcte n'est pas un choix, c'est la règle du dépôt. 🔴 **Second défaut, dans 5 des 11 fichiers** : le **nom d'une Question de FAQ** était corrompu par une purge automatisée antérieure et servi tel quel aux crawlers — `"Atendem Atendimento — ligue 928 484 451/7d?"` (`sinais-alerta-casa-antiga`, `top-10-fugas-mais-comuns`, `top-10-razoes-contratar-canalizador`) et `"Atendem Atendemos 24h/7d?"` (`glossario-canalizacao`, `glossario-eletricidade`). **Un numéro de téléphone injecté au milieu d'un intitulé de question.** Restauré depuis le **jumeau intact du même fichier** : chacun de ces 5 fichiers porte un **premier** bloc `FAQPage`, deux lignes plus haut, avec la question saine `"Atendem 24h/7d?"` → zéro invention (R4), donneur à distance de deux lignes. ⚠️ **Faux positifs écartés par requalification en lecture** : le prédicat « nom de nœud contenant un numéro de téléphone » sortait **10 fichiers**, dont **6 faux positifs** — `"Ligue 928 484 451 e descreva os sintomas"`, `"Ligue 928 484 451 em segurança"`, `"Documentar com foto e ligar 928 484 451"` sont des **noms d'étapes `HowToStep`**, où le numéro est légitime. **Un numéro dans un `name` n'est une anomalie que si le `name` est celui d'une `Question`.** 🔎 **Découvertes consignées, non traitées** : (a) **les 5 mêmes fichiers portent DEUX blocs `FAQPage` avec les MÊMES questions** — un ancien (555 o) et un récent (766 o) qui, lui, énonce les 4 engagements de `PRICING.md` verbatim. Ils **ne sont pas byte-identiques**, donc le prédicat de retrait mécanique (md5 par bloc, méthode EU #314) **ne s'applique pas** : même situation que les deux `FAQPage` de `precos.html` sur ENR. **Arbitrage.** (b) **`Diagnóstico por telefone em poucos minutos` = 5 134 occurrences / 1 084 fichiers** — `poucos minutos` a déjà été purgé comme R145 sur `calculadora-de-preco.html` le 14/08 ; à cette échelle c'est un **batch → GO**. (c) **10 pages ont une meta description dont une phrase entière est répétée deux fois** — surface SERP, sans GO. (d) `garantimos atenção após contacto telefónico` : **20 occ / 9 fichiers de production**, dont **6 en meta description**. | R4 (zéro invention — la forme `"@type"` est unanime dans le dépôt ; les noms de Question sont restaurés depuis le jumeau intact **du même fichier**), R11/R12 (données structurées non émises en production = violation active), R8 (témoins avant/après comptés **en Python sur le contenu**, + revalidation par parsing des 6 960 blocs `ld+json` du dépôt), R145 (aucune formulation bannie introduite), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree `~/work/Sites/_worktrees/loop-cu-20260824` ; checkout partagé **non touché**) | **12 commits, 11 fichiers de production** + `SEO_PLAN.md`. **Témoins R8** : `"type"` nu **27 → 0** sur les 11 fichiers **et 27 → 0 sur tout le dépôt** · `"@@type"` accidentels **0** · noms de Question corrompus **5 → 0** · `git diff --numstat` = **1/1 ou 2/2 par fichier** (substitution ligne pour ligne, **aucun bloc supprimé**) · revalidation : **6 960 blocs `ld+json` parsés, 5 invalides** — les 5 sont dans `blog/canalizador-urgente-guia-completo.html`, **déjà corrigé par la PR #269 ouverte**, donc hors périmètre. Contrôle de réservation passé **avant** le calcul du périmètre : 5 PR ouvertes, 119 chemins, **aucun des 11 fichiers**. Branche `loop/2026-08-24-cu-sweep` depuis `origin/main`. | ⏳ PR ouverte |
| 2026-08-23 | cowork-loop | **`contactos.html` — la SEULE page du dépôt servie en mode quirks : DOCTYPE, `<html lang>` et `<head>` absents** | Rang 1 de la file du 22/08 (contrôler `comparacao.html` d'EU) relève du repo EU, traité là-bas ; rang 2 (suite du sweep `LECONS.md`) mis de côté au profit d'un **balayage structurel systématique de tout le dépôt**, transposé de la découverte faite sur ENR le même run. Balayage sur **2 454 fichiers HTML** (hors `.git`, `node_modules`, `_archive`, `_audit`, `_indexing`) : équilibre de 10 balises, validité JSON de chaque bloc `application/ld+json`, doublons byte-à-byte, présence du DOCTYPE et de `<html lang>`. 🔎 **Résultat : CU est le plus sain des 4 repos** — seulement **3 fichiers** à problème sur 2 454, et l'un des trois (`blog/canalizador-urgente-guia-completo.html`, 5 JSON-LD invalides) est **déjà pris par la PR #269**. 🔴 **Le défaut traité** : `contactos.html` commençait directement par `<script type="application/ld+json">` — **ni `<!DOCTYPE html>`, ni `<html lang="pt-PT">`, ni `<head>`** — alors qu'il porte bien un `</head>` (L42) et un `</html>` (L75). Témoins : `<html>` **0/1**, `<head>` **0/1**. **Deux conséquences réelles en production, sur une money page de conversion** : (a) sans DOCTYPE le navigateur bascule en **mode quirks** (modèle de boîte hérité, rendu différent de toutes les autres pages du site) ; (b) sans `<html lang>` la page ne porte **aucun signal de langue** pour les crawlers ni pour les lecteurs d'écran. **Preuve d'unanimité** : sur les **2 454** fichiers HTML du dépôt, `contactos.html` est le **seul** sans DOCTYPE **et** le **seul** sans `<html lang>`. Sur les 2 075 pages racine, les prologues se répartissent en 5 variantes (1 078 `<!DOCTYPE html><html lang="pt-PT">`, 475 et 410 avec sauts de ligne, 110 en `<!doctype>` minuscule, 1 en `lang="pt"`) : **`lang="pt-PT"` est unanime à 2 074/2 075**. Prologue restauré **verbatim** depuis `sobre.html` (page institutionnelle sœur, même famille de gabarit) — **preuve : le prologue résultant est byte-identique à celui de `sobre.html`**, forme partagée à l'identique par 410 pages du dépôt. ⚠️ **Non touchés délibérément, sur ce même fichier** : (a) les occurrences de `mediante confirmação por telefone` — **la PR #264 porte exactement sur cette clause**, patcher ici créerait un conflit ; (b) `Z1 (0-30 km): . Z2 (31-50 km): .` — des **valeurs de délai purgées ont laissé des deux-points suivis d'un point**, servis en clair ; y écrire un délai serait une invention (R4/R145) → consigné en file ; (c) `menos de Atendemos 24h/7 dias, mediante confirmação…` — phrase grammaticalement cassée par la même purge, même famille que (b). | R4 (zéro invention — prologue transplanté **verbatim** d'un jumeau du dépôt, et **byte-identique** après patch ; les défauts non sourçables sont consignés, pas comblés), R11/R12 (défaut actif en production = priorité), R8 (témoins avant/après comptés **en Python sur le contenu**, jamais via `git` depuis le sandbox), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree ; checkout partagé **non touché**) | 2 commits, 1 fichier de production + `SEO_PLAN.md`. **Témoins R8** sur `contactos.html` : `<html>` **0/1 → 1/1** · `<head>` **0/1 → 1/1** · DOCTYPE **absent → présent** · `<html lang>` **0 → 1** · `<body>` 1/1, `<header>` 1/1, `<main>` 1/1, `<section>` 6/6 **inchangés** · prologue byte-identique à `sobre.html` **True**. Balayage global du dépôt : fichiers à problème **3 / 2 454** (1 pris par #269, 1 traité ici, 1 consigné). Contrôle PR ouvertes avant patch : **105 chemins** pris par les 5 PR ouvertes (#272, #269, #268, #264, #243) — `contactos.html` **non pris**. Branche `loop/2026-08-23-cu-html-structure` depuis `origin/main`, en **worktree** sous `~/work/Sites/_worktrees/loop-cu-20260823`. | ⏳ PR ouverte |
| 2026-08-22 | cowork-loop | **Rang 2 de la file — 3 chaînes françaises du corpus INTERDIT : la cause était une COPIE PÉRIMÉE du corps de page** | Rang 1 (sweep `LECONS.md`) et rang 2 (chaînes françaises) sont le même défaut sur `comparacao.html`. 🔴 **La page contenait DEUX copies complètes de son corps**, chacune avec son propre `<header>`, `<h1>`, `<main>` et `<section>` : **L27 (périmée, corrompue) et L29 (corrigée)**. Les deux portent exactement les **mêmes 7 `<h2>`**. 🔎 **Contrôle décisif avant patch** : les **seuls** segments présents dans L27 et absents de L29 sont **les 7 fragments corrompus eux-mêmes** — les 3 chaînes françaises interdites (`Marque grande`, `satisfait ou refait`, `parle uniquement`), les 4 `<li>` en pt-BR `Você` à **parenthèse non fermée** (phrases tronquées), et le CTA `Compare vous-même. Premier service garanti satisfait ou refait gratuitement.` (**garantie non sourcée**, R4/R11). Rien d'autre. ➡️ **Le correctif n'est donc pas de rapiécer L27 mais de la supprimer** : zéro perte de contenu, prouvée segment par segment, et les 3 chaînes tombent d'un coup **avec** les 4 `Você` — sans consommer le GO du batch `Você` (c'est une déduplication, pas une réécriture de pronom). ⚠️ **Le déséquilibre `<main>` 2/1 était invisible à tous les compteurs** : `<header>` 2/2 et `<h1>` 2/2 étaient *équilibrés* — c'est-à-dire **dupliqués**, pas cassés. Un `<h1>` en double et un `<main>` non fermé sur une page live. 🔎 **Propagation vérifiée sur les 4 repos** : **ENR porte TROIS copies** (`<h1>` ×3, `<main>` 3/1, `<section>` 3/1) — traité en PR jumelle ; **CNR est propre** (1/1/1) ; **EU à contrôler**. | R4/R11 (garantie `satisfait ou refait` non sourcée — retrait, jamais réécriture ; zéro invention : la copie conservée est celle déjà présente sur disque), `LECONS.md` corpus INTERDIT (chaînes françaises, pt-BR `Você`), R8 (témoins avant/après, 1 motif par commande, avant mesuré sur `origin/main` intact), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree ; checkout partagé sur `feat/cu-rankpush-canalizador-urgente-t_a9810c1c`, 32 fichiers non commités, **non touché**) | 1 commit, 1 fichier. **Témoins R8** : `Marque grande` **1→0** · `satisfait ou refait` **1→0** · `parle uniquement` **1→0** · `Compare vous-même` **1→0** · `Você` **4→0** · `délai`/`spécifique`/`nationale` **4→0** · `Marca grande nacional` **1→1** (préservée) · `<h1>` **2→1** · `<header>` **2/2→1/1** · `<main>` **2/1→1/1** · `<section>` **3/2→2/2** · `<div>` 17/17→10/10 · `<li>` 41/41→22/22. Contrôle PR ouvertes avant patch : 4 PR (#269, #268, #264, #243), `comparacao.html` **non pris**. Branche `loop/2026-08-22-cu-chaines-francaises` depuis `origin/main`, en **worktree** sous `~/work/Sites/_worktrees/loop-2026-08-22/cu`. | ⏳ PR ouverte |
...
Le `context.md` du 12/08 concluait que les 21 fichiers portant ` conforme zona` **hors JSON-LD** l'avaient « dans une phrase légitime et grammaticale ». **Vérifié fichier par fichier sur `origin/main` : vrai pour 20 sur 21.** L'exception unique est **`contactos.html`** — et c'est la page la plus à enjeu du lot (racine, money page) :
  ...
  - `« ... (média ). Por email: conforme zona úteis. »`
...
### 2026-08-23 — t_1b18b0c3 — Rank-push 3/3 sur `canalizador urgente` — **consolidation du cluster autour du guide (3 nouveaux liens entrants)**

- **Contexte** : GSC fenêtre 28j terminée 2026-08-23 = **3 impressions / 0 clic / position 30.7** sur `canalizador urgente` (CPC=14.63 EUR, vol=170, score 2487.10 — broad-money H1). Page guide broad-money `/blog/canalizador-urgente-guia-completo` (créée 11/08 PR t_b63ca193) existe mais **orpheline totale** sur `origin/main` SHA `03842f1ee` : la PR précédente t_a9810c1c (commit `2420d767f`) avait ajouté 1 lien homepage↔guide + bumpé dateModified, mais le commit **n'est PAS ancêtre d'origin/main** (vérifié `git merge-base --is-ancestor 2420d767f origin/main` = false) → la PR n'a jamais été mergée. **État réel sur main : 0 lien entrant** vers la page guide (vérifié `git grep canalizador-urgente-guia-completo origin/main -- '*.html'` = seul le `<link rel="canonical">` de la page cible sort). Les 3 autres pages broad-money adjacentes (`comparacao.html`, `calculadora-de-preco.html`, `perguntas-frequentes.html`) ciblent toutes l'intention large sans consolider le cluster.
- **Diagnostic anti-doublon** : la page guide existe, FAQPage+Service+BlogPosting en place, sitemap OK. La **fenêtre GSC s'allonge** (3 impr vs 1 impr le 20/08 — mesure observée dans la branche t_a9810c1c) — Google découvre la page mais la position **baisse** (22 → 30.7). Hypothèse = autorité thématique insuffisante (0 lien entrant sur main = pas de signal). Levier suivant logique = densifier le maillage entrant **depuis les pages satellites broad-money** (comparacao, calculadora, FAQ) + réintroduire le signal racine homepage→guide (puisque la PR t_a9810c1c n'a jamais été mergée, je consolide ici les 2 leviers dans le même scope strict : 4 liens entrants pour le guide).
- **Travail effectué** : **consolidation trilogie + rattrapage t_a9810c1c** en scope strict : (1) **1 ligne homepage→guide** dans section « Leituras úteis do blog » de `index.html` (rattrapage du diff t_a9810c1c qui n'avait jamais été mergé) ; (2) **1 carte dans « Recursos Úteis »** (grille 4→5 cartes) de chacune des 3 pages satellites broad-money (`comparacao.html`, `calculadora-de-preco.html`, `perguntas-frequentes.html`), pointant vers `/blog/canalizador-urgente-guia-completo` avec texte d'ancrage « 🚨 Guia Canalizador Urgente — Como funciona, quando chamar e quanto custa » ; (3) **bump `dateModified` JSON-LD** `2026-08-11 → 2026-08-23` (signal de fraîcheur post-cluster) ; (4) **bump `<lastmod>` sitemap-blog** `2026-08-11 → 2026-08-23` (cohérence sitemap ↔ page). **Total : 4 liens entrants nouveaux** pour le guide (0 → 4 sur main).
- **Témoins R8** (6 fichiers, 28 insertions / 2 deletions) :
  - `index.html` : `href="/blog/canalizador-urgente-guia-completo"` 0 → 1 (insertion 1 ligne dans Leituras úteis blog ligne 397)
  - `comparacao.html` : `href="/blog/canalizador-urgente-guia-completo"` 0 → 1 (insertion 1 ligne dans Recursos Úteis ligne 46)
  - `calculadora-de-preco.html` : `href="/blog/canalizador-urgente-guia-completo"` 0 → 1 (insertion 1 ligne dans Recursos Úteis ligne 167)
  - `perguntas-frequentes.html` : `href="/blog/canalizador-urgente-guia-completo"` 0 → 1 (insertion 1 ligne dans Recursos Úteis ligne 88)
  - `blog/canalizador-urgente-guia-completo.html` : `"dateModified":"2026-08-11"` 1 → 0, `"dateModified":"2026-08-23"` 0 → 1
  - `sitemap-blog.xml` : `<lastmod>2026-08-11</lastmod>` (URL guide) 1 → 0, `<lastmod>2026-08-23</lastmod>` 0 → 1
  - `git diff --numstat` : **6 fichiers modifiés, 28 insertions, 2 deletions** — scope strict.
  - Total liens entrants guide broad-money sur main : **0 → 4** (×4 densité de signal thématique : homepage + 3 satellites distincts).
- **Conformité** :
  - R12 post-08-07-08 : NAP inchangé (`+351****4451` non touché — Leçon #351 sur E.164 pré-existant hors scope), prix 65 €/h + Z1–Z6 + +50 % non touchés (le texte d'ancrage n'introduit aucun prix).
  - R145 : **aucun délai chiffré introduit** (texte d'ancrage « como funciona, quando chamar e quanto custa » = formulation générique, aucune minute/heure). ⚠️ **R145 pré-existant observé mais hors scope** : (a) `comparacao.html` L27 contient 4× « mediante confirmação por telefone » (motif banni post-purge #348) + colonnes `Tempo médio resposta Z1` avec valeur « Atendimento mediante confirmação por telefone » (banni). (b) `calculadora-de-preco.html` JSON-LD FAQPage L33 « mediante confirmação por telefone, 65 EUR/h » (banni) + L157 body. (c) `perguntas-frequentes.html` L40 « 30 a 90 minutos » (R145 interdit délai chiffré). **Toutes ces violations sont TECH DEBT hérité (PR #271 comparacao.html fixait déjà la copie périmée mais pas la copie vivante), NON introduit par ce PR** → à traiter dans un Kanban dédié R145 batch (cf. leçon t_45ec27ae).
  - R11 / §14 CU : zéro invention, zéro DGEG/TRIESP/wallbox/carregador ajouté (vérification grep avant patch).
  - Annexe A : zéro « je/eu » introduit.
  - R6 : zéro force-push, branche `feat/cu-rankpush-canalizador-urgente-t_1b18b0c3` depuis `origin/main` SHA `03842f1ee` (post-merge `03842f1ee` chore context 2026-08-22).
- **Branche** : `feat/cu-rankpush-canalizador-urgente-t_1b18b0c3` depuis `origin/main` SHA `03842f1ee` — **STOP MERGE** validation Philippe requise (R7).
- **Mesure** : cron `gsc-trajectoire-cron.sh` dim 22h → J+7/J+14 suivre impressions/clics/position sur `canalizador urgente`. Hypothèse = la densification du cluster (1→4 liens entrants, distribution sur 3 pages thématiques distinctes comparacao/calculadora/FAQ) doit renforcer le signal d'autorité thématique autour du pilier broad-money. Si confirmé → leçon « pour un pilier broad-money en position 20-30, le levier maillage entrant depuis pages satellites broad-money > modification contenu cible ». Si pas confirmé → escalader CEO (backlinks externes depuis canalizador-norte-reparos.pt, ou acceptation position concurrentielle forte).
- **Leçon consolidée trilogie** : (a) t_b63ca193 « créer une page pilier broad dédiée quand le renforcement d'une page existante échoue ». (b) t_a9810c1c « une page pilier broad sans maillage depuis la racine reste orpheline ». (c) t_1b18b0c3 « un seul lien entrant racine = signal insuffisant face aux positions 20-30 — densifier le cluster avec pages satellites broad-money (×3-4 liens entrants depuis pages thématiques distinctes) ». Doctrine finale = **« page pilier broad-money = contenu solide + maillage entrant multi-sources (racine + satellites broad-money) »**.
...
### 2026-08-11 — t_b9694e7e — Rank-push `canalizador 24 horas` (CPC=18.63 EUR, vol=320) — supersede PR #228

- **Brief** : query money `canalizador 24 horas` sur CU, DataForSEO vol=320/mois, CPC=18.63 EUR, score=5961.60 — **la requête la plus chère du marché portugais plomberie**. GSC 28j terminé 2026-08-11 = 0 impression / 0 clic / position None → **GAP H1**.
- **Anti-doublon vérifié** : PR draft #228 (t_8d67d0b4) dormante depuis 04/08 sur le même scope. PR #243 (t_b9694e7e, cette mission) supersede #228 en intégrant les corrections R12 postérieures (ruling 2026-07-08 sur `orçamento por escrito`) et 2 sections valeur ajoutée (zones servidas explicites + confiance avant de contratar).
- **Modifs** : title/meta/H1/og/direct-answer/JSON-LD BlogPosting alignés sur `canalizador 24 horas` ; prix issus de `PRICING.md` (65 €/h, Z1=15 €, Z2=25 €, Z3=35 €, Z4=45 €, Z5=55 €, Z6=65 €, +50 %) ; NAP `+351 928 484 451` fil rouge (8 occurrences, vs 3 avant) ; +2 JSON-LD (BreadcrumbList 3 items + HowTo 3 passos) ; FAQ 6→11 (+5 longues traînes Money : samedi soir, atendimento 24h réel, zones Trás-os-Montes, Bragança Z2, dimanche) ; +2 sections (Zonas servidas 24h / Confiança avant contratar) ; +6 outbound links concelhos (braganca, mirandela, vila-real, chaves, mogadouro, vimioso) ; +1 ligne dans `index.html` "Leituras úteis do blog" ; sitemap-blog.xml lastmod 2026-08-04 → 2026-08-11.
- **Conformité** : R11 zéro invention ✅ ; R12 transparence prix HAUT + règle postérieure 2026-07-08 respectée (`preço confirmado antes de qualquer intervenção`, zéro `orçamento por escrito`) ✅ ; phrase `fala sempre com a mesma pessoa, não um call center` ✅ ; R145 aucun délai chiffré (`24h/7 dias mediante confirmação por telefone` OK) ✅ ; §14 CU = 0 DGEG/TRIESP/wallbox/carregador ✅ ; Annexe A "nous" toujours ✅ ; R7 STOP MERGE ✅ (PR draft only).
- **Témoins R8** : mots 2444→3566 (+46 %) · "canalizador 24 horas" 5→19 (+280 %) · "24 horas" 6→23 (+280 %) · "65 €/h" 10→11 · Z[1-6] 12→22 (+83 %) · NAP 3→8 (+167 %) · JSON-LD 3→5 · FAQ visible 6→11 · outbound concelhos 0→6 · 0 "orçamento por escrito" · 0 DGEG/TRIESP/wallbox.
- **PR** : draft #243 sur branche `feat/cu-rankpush-canalizador-24-horas-t_b9694e7e`, base `origin/main` (a272d87f5), head `1e184edec`, scope strict 3 fichiers +43/-13. PR #228 supersédée — Filipe tranchera entre #243 et #228. **Zéro merge sans GO Filipe (R7).**
- **Mesure impact** : J+7 et J+14 via `gsc-trajectoire-cron.sh` (cron dim 22h, id 8e0fd9b3e269). Si uplift confirmé sur `canalizador 24 horas`, batch envisageable sur autres pages piliers du même profil (fuga de agua, cano rebentado, etc.).

---

### 2026-09-01 — t_916db1b1 — Audit SEO_PLAN ligne 85 « Rôle de ce site = 2e slot organique sur canalizador <ville> » — NO-OP légitime confirmé

- **Brief** : traiter le chantier vivant marqué ligne 85 de `SEO_PLAN.md` (cf. tâche dispatchée pool-keeper). La ligne annonce « Prérequis refonte Transparence Radicale = **quasi ✅ FAIT** au 2026-08-17 (A1 homepage + A2 services × district + A3 570 services + A4 1827 villes + P0 70→65 €/h ×1476 = ~3 900 fichiers doctrine-conformes) ; gisements résiduels = traces R145 `mediante confirmação` + stale 🔴 items §ÉTAT lignes 104-108 (audit Kanban `t_bb4ef8ea` 2026-08-17) ». Vérifier si (a) le chantier est encore APPLICABLE, (b) le bloc 🔴 119-127 reflète l'état réel main, (c) le gisement `mediante confirmação` est toujours présent.
- **Recompte live `origin/main` (2026-09-01)** :
  - `index.html` : **492 lignes / 118 éléments structurants** (cible Doctrine §12.1 « homepage non-squelettique » : OK, vs claim obsolète « 16-39 éléments » ligne 119 datant du **2026-06-28** pré-refonte A1 — leçon #447).
  - **« 65 € » × 8**, **« Z1-Z6 » × 17** → grille prix canonique Doctrine §12.1 INTACTE (lignes 120 obsolètes).
  - **« fala sempre com a mesma pessoa » = 0** ; **« nossa equipa/nossos técnicos » × 7** → **pivot R152 (verrouillé 30/06/2026)** déjà appliqué : formulation collective prime, le claim solo est banni (arbitrage Filipe 29/07 — AGENTS.md Annexe A prime sur §12.2). Ligne 121 caduque par override R152, pas par oubli.
  - **« Ridgid K9-102 » × 2**, **« ROLeak » × 2** → équipement EXACT Doctrine §12.6 INTACT (ligne 122 obsolète).
  - **« FAQPage » × 2** → schema.org FAQPage INTACT (lignes 123-124 obsolètes).
  - **8/8 pages `/zonas/` prioritaires PRÉSENTES** : `canalizador-urgente-{braganca,mirandela,vila-real,chaves,miranda-do-douro,mogadouro,vinhais,lamego}.html` (PR #46 merge, A2 ✅ FAIT) ; claim ligne 125 « Pages /zonas/ = 0 » obsolète.
  - `public/index.html` toujours présent mais **B2 PR #67 squash `4144f002a` MERGED 01/07/2026** (synchronisation copie conforme de `index.html`) → doublon non-canonique neutralisé via canonical + sitemap (à re-confirmer en passe CEO).
  - **Gisement `mediante confirmação` global** : **299 occurrences** restantes sur `origin/main -- '*.html'` (gisement RÉEL encore vivant, déjà listé ligne 85 + M0 ligne 38 STOP Filipe « purger ou tolérer »).
- **Verdict NO-OP légitime** : chantier ligne 85 = **déjà FAIT** par PR #45 (29/06/2026) + PR #46 + PR #48 + PR #49 + PR #40 + PR #67 (chronologie de la refonte Doctrine Transparence Radicale). Le bloc « Faiblesses SEO/GEO CRITIQUES » lignes 118-127 documente l'état T₀ 2026-06-28 (audit initial pré-A1) et n'a jamais été re-synchronisé avec l'état réel post-PR. La mesure « 16-39 éléments » date de T₀.
- **Décision scope strict** : **0 ligne SEO_PLAN.md modifiée** sur cette tâche. (a) La modification relèverait d'une **carte fille CEO/docs-only** dédiée (assignee = `default` ou CEO), à matérialiser après GO merge PR #309 (rang-push query money) pour éviter tout drift sur PR en attente. (b) Cohérent avec la décision déjà prise par `t_bfbf0caf` (draft JOURNAL.md non commité, 2026-09-01 antérieur à ce run) — un worker ne touche pas SEO_PLAN.md sur une branche code en PR DRAFT. (c) Le gisement `mediante confirmação` (299 occ.) reste en attente décision Filipe (M0 STOP ligne 38) → NE PAS purger sans GO.
- **Conformité** : R11 zéro invention ✅ (aucun contenu fabriqué) ; R12 Doctrine Transparence Radicale ✅ (recompte live confirme intégrité prix/équipement/FAQPage/schema.org/zonas) ; R145 zéro délai chiffré (pas de modification) ; Annexe A formulation collective ✅ (déjà pivoté R152) ; R7 STOP MERGE ✅ (0 merge) ; R3 audit lecture-seule ✅ ; R8 témoins git show+grep comptabilisés ; R01 recompte live AVANT toute affirmation chiffrée ✅ (leçon #447 appliquée).
- **Carte fille à matérialiser** (par pool-keeper / CEO, hors scope worker) : `docs/cu-seoplan-ligne85-statut-fresh-2026-09-01` assignee=`default` — corps = « Recompte live 2026-09-01 confirme bloc 🔴 119-127 OBSOLÈTE de fait depuis PR #45 (2026-06-29). Consigner en CEO/docs-only pass : transformer 🔴 → ✅ NO-OP légitime groupé (alignement wording CNR ligne 34, leçon #447). Décision séparée pour gisement `mediante confirmação` 299 occ. (M0 STOP Filipe). »
- **LEÇON (extension `kanban-stale-cu-homepage-count-2026-09-01-03`)** : un bloc « Faiblesses » dans `SEO_PLAN.md` documente un instant T₀ ; sans revalidation chiffrée à chaque tâche sur un diagnostic figé, un worker peut rouvrir un chantier déjà clôturé. **Réflexe** : avant toute action sur ligne marquée 🔴/🟠/🟡, recompter LIVE les claims sur `origin/main` (R01 + leçon #447). Si les claims sont obsolètes, le chantier est **NO-OP légitime** et l'audit va dans JOURNAL.md — pas dans SEO_PLAN.md sur une branche code en PR DRAFT.
- **PR** : PR DRAFT #310 ouverte (https://github.com/taffrand-gif/canalizador-urgente/pull/310) sur branche `docs/cu-seoplan-ligne85-t_916db1b1` depuis `origin/main@a6ac26620`, commit unique `15ee78f54`, 1 fichier `JOURNAL.md` +21/-0. **Zéro merge sans GO Filipe (R7).**

### 2026-09-18 — t_318b6f69 — Audit SEO_PLAN ligne 137 « ✅ L.119 Homepage squelettique 16-39 éléments — clos par PR #45 » — NO-OP légitime revalidé J+17

- **Brief** : traiter le chantier vivant marqué ligne 137 de `SEO_PLAN.md` (cf. tâche dispatchée pool-keeper). La ligne annonce « ✅ **L.119 (ex-🔴) Homepage squelettique 16-39 éléments** — clos par PR #45 (29/06/2026). Recompte 2026-09-01 = **235 éléments HTML** sur 492 lignes (`index.html`). Cible 194 du 17/08 dépassée. ». Vérifier si le chantier est encore APPLICABLE J+17 après `t_a07cc45d` (2026-09-01, PR #312 squash `3d7200ddd` MERGED — passage 🔴→✅ NO-OP légitime groupé sur les 9 entrées 🔴/🟠 lignes 119-127).
- **Recompte live `origin/main@06e77261a` (2026-09-18, base PR #312 + #310 mergées + X-QUOTEPATH #330 merge 2026-09-18)** :
  - `index.html` : **492 lignes / 235 éléments HTML** ✅ (exact match SEO_PLAN L.137 — pas de drift en 17 jours)
  - **Grille prix canonique Doctrine §12.1 INTACTE** : `65 €/h` × 4 (l.38, 379, 439) + `65€/h` × 1 sans espace (l.39 og:title) + `65 €/hora` × 1 (l.48 JSON-LD) = **6 occurrences canoniques** (SEO_PLAN L.138 annonçait ×5 — sur-comptage acceptable, plancher canonique ×5 confirmé) ; `Zona 1..6` × 6 chacun (zone-cards l.284-289) ; `+50%` × 2 (l.292 + l.413 FAQ) ; `orçamento por escrito` × 3 (l.379, l.398, l.439)
  - **schema.org FAQPage × 2** (l.48 JSON-LD + meta) — INTACT
  - **Équipement EXACT Doctrine §12.6 INTACT** : `Ridgid K9-102` × 2, `ROLeak` × 2, `FLIR E96` × 2
  - **Annexe A formulation collective ✅** : `fala sempre com a mesma pessoa` = **0** (banni par R152 30/06/2026) ; `nossa equipa/nossos técnicos` × 7 — formulation collective prime
  - **NAP inchangé** : `+351 928 484 451` × 15 (fil rouge consolidé vs ×3 initial 29/06 — gain documentaire sur 17 jours)
  - **8/8 pages `/zonas/` prioritaires PRÉSENTES** : `canalizador-urgente-{braganca,mirandela,vila-real,chaves,miranda-do-douro,mogadouro,vinhais,lamego}.html` (PR #46 merge A2 ✅ inchangé depuis 2026-08)
  - **`public/index.html` toujours présent** + PR #67 squash `4144f002a` MERGED 01/07/2026 tracé dans JOURNAL × 5 (B2 doublon neutralisé via canonical + sitemap — inchangé)
  - **Gisement `mediante confirmação` homepage** : **0 occurrence dans `index.html`** (R145 purge locale déjà appliquée — cohérent avec le chantier X-R145 antérieur). Le gisement global historique de 299 occurrences (mesuré 2026-09-01 sur l'arbre complet) reste hors scope de cette tâche.
- **Verdict NO-OP légitime revalidé** : chantier ligne 137 = **toujours ✅ FAIT** (J+17 depuis la dernière consigne par `t_a07cc45d`). Aucune régression sur les 5 invariants depuis PR #45 (29/06/2026) + PR #312 (01/09/2026). La consigne SEO_PLAN L.137 elle-même est exacte et cohérente avec `origin/main`. **Zéro modification SEO_PLAN.md requise.**
- **Décision scope strict** : **0 ligne SEO_PLAN.md modifiée** sur cette tâche. (a) `t_a07cc45d` (2026-09-01) a déjà transformé 🔴→✅ groupé sur les 9 entrées 🔴/🟠 lignes 119-127 (PR #312 squash `3d7200ddd` MERGED). Re-modifier serait du drift sur du travail mergé. (b) Cohérent avec `t_916db1b1` (2026-09-01, PR #310) et `t_bfbf0caf` (2026-09-01, draft JOURNAL.md non commité antérieur) — un worker ne touche pas SEO_PLAN.md sur une branche code en PR DRAFT, et ce pool re-dispatch est le **5ᵉ passage** sur le même bloc en 17 jours (cf. `t_bb4ef8ea`, `t_33a93e6c`, `t_4a1bce6d` du 17/08 + `t_a07cc45d`, `t_916db1b1` du 01/09). (c) Le gisement `mediante confirmação` global reste en attente décision Filipe (M0 STOP ligne 38) — hors scope homepage-only ici.
- **Conformité** : R11 zéro invention ✅ (aucun contenu fabriqué ; décompte lu dans git + filesystem recompte 2026-09-18) ; R12 Doctrine Transparence Radicale ✅ (recompte live confirme intégrité prix/équipement/FAQPage/schema.org/zonas — exact match SEO_PLAN L.137-138) ; R145 zéro délai chiffré ✅ (pas de modification) ; Annexe A formulation collective ✅ (override R152 pivoté 30/06/2026 confirmé) ; R7 STOP MERGE ✅ (0 merge) ; R3 audit lecture-seule ✅ ; R8 témoins git show+grep comptabilisés ✅ (13/13 invariants vérifiés) ; R01 recompte live AVANT toute affirmation chiffrée ✅ (leçon #447 appliquée) ; leçon extension `kanban-stale-cu-homepage-count-2026-09-01-03` ✅ (déjà appliquée 17 jours plus tôt — revalidation confirme qu'elle tient toujours).
- **LEÇON (extension `kanban-stale-cu-homepage-count-2026-09-18-04`)** : un même chantier `SEO_PLAN.md` peut être re-dispatché **5 fois en 17 jours** par le pool-keeper sans qu'aucun worker n'ait produit de modification SEO_PLAN (toutes les passes sont arrivées à la même conclusion NO-OP légitime). C'est attendu et correct : le coût d'une revalidation est nul, le coût d'une régression sur PR mergée est réel. **Réflexe** : pour les chantiers SEO_PLAN déjà marqués ✅ après passage CEO/docs-only, accepter le NO-OP légitime comme **résultat valide**, pas comme échec — la trace JOURNAL.md suffit à prouver que la garde a été tenue. **Pas de carte fille à matérialiser cette fois** (contrairement à `t_916db1b1` qui suggérait `docs/cu-seoplan-ligne85-statut-fresh` — ici L.137 a déjà été transformée ✅ par `t_a07cc45d`, donc rien à faire côté SEO_PLAN). **Métrique** : 5 passages × ~3 min recompte live = ~15 minutes cumulées de garde sur un chantier clos, vs coût marginal d'un faux positif de modification SEO_PLAN sur PR mergée = contamination de 3 PRs en cascade. Le ratio plaide pour la revalidation systématique.
- **PR** : PR DRAFT **#336** ouverte (https://github.com/taffrand-gif/canalizador-urgente/pull/336) sur branche `docs/cu-seoplan-l137-t_318b6f69` depuis `origin/main@06e77261a`, commit unique `78d19373e`, 2 fichiers `JOURNAL.md` +19/-0 + `LECONS.md` +15/-0. **Zéro merge sans GO Filipe (R7).**
