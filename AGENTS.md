# AGENTS.md — Règles Verrouillées (repo `canalizador-urgente`)
...
- Site satellite : `canalizador-urgente.pt` (backlink vers `canalizador-norte-reparos.pt` = `norte-reparos` repo)
...
- **AUCUN DOCUMENT ÉMIS (ruling Filipe 2026-07-08, verrouillé)** : Filipe fait **uniquement le travail** (instalação, reparação, diagnóstico). Il ne délivre **ni certificat, ni orçamento por escrito (de conformidade), ni ficha**. **INTERDIT** sur toute page : « emitimos/fazemos certificação », « orçamento por escrito », « trabalho profissional », « emissão de certificado », « instalações certificadas ». **INTERDIT AUSSI** toute mention du statut DGEG (« em curso », « enregistrement en cours », « aguardando », n° de dossier). ⚠️ Site PLOMBIER : **jamais** de claim de certification/inspection ÉLECTRIQUE (cross-domaine). Info éducative neutre OK, jamais « nous le faisons ». Reframe = décrire le **travail réel**.
